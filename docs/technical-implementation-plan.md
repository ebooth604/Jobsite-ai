...
4. **Cloud provider (AWS vs. Azure)** — **settled.** Founder-confirmed 2026-08-24. [ADR-0001](adr/0001-cloud-provider.md) picks AWS `ca-central-1` and records why: both regions satisfy residency, so residency does not decide it; the §3 stack already leans AWS (ECS/Fargate, Cognito). Reversal was near zero and rises steeply once IAM and networking exist, so confirming before any resource was applied used that cheap window rather than spending it. SR&ED is explicitly not a reason either way — that is a question for the founders' accountant, and if it favours Azure it should override the ADR.
