...
4. **Training data:** starts from the 3 BC firms' real photo sets + as-built quantities referenced in Business Plan §16 (days 31–60 technical spike) — this is the seed dataset. Corrections logged in production become the ongoing training signal.
...
- **Accuracy harness:** automated comparison of model output against held-out as-built ground truth, tracked per trade, per model version, over time. This is the data behind the Q1/Q2 ±15%/±10% milestones and the "accuracy report" sales asset (Business Plan §5.2, §5.3).
