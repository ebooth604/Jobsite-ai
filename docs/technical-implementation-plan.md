...
| **Cloud provider** | **Defaulted to AWS `ca-central-1` (Montreal)** — see [ADR-0001](adr/0001-cloud-provider.md); Azure Canada Central remains the alternative, and the founders should confirm or override before infra work starts | Business Plan §4.3 and §8 make Canadian data residency a hard, contractual commitment, not a preference |
...
4. **Training data — build it so no single firm is load-bearing.** Training data comes from four sources, deliberately ordered by how little they depend on a customer's cooperation:
   ...
   - **(d) Simulated / synthetic data (open option, training only).** Keep this on the table rather than ruling it out. Rendered scenes from BIM or Revit models are the most promising form, because the installed quantity is exact by construction — the model *is* the ground truth, with no measuring error and no firm to ask. Procedurally generated layouts and heavy augmentation of real photo sets (lighting, occlusion, angle, dust, partial framing) serve the same purpose more cheaply. Two places this earns its keep: **cold start**, producing a trainable model before the first self-measured site is booked, and **rare conditions** that are expensive to wait for in the field — bad light, half-blocked views, unusual layouts, the cases where abstention behaviour actually matters. Electrical rough-in suits this well, since conduit runs and device boxes are routinely modelled; concrete forming much less so, and the plan should expect a worse return there.
   ...
   **The hard line on (d): simulated data may train a model, and may never measure one.** It never enters the held-out set, and no accuracy figure quoted internally or to a customer may rest on it. The sim-to-real gap is exactly the kind of error that looks like accuracy right up until a foreman photographs a real wall — a model can score beautifully on renders and fail on the first job. So track the synthetic share of each model version's training mix as a first-class field alongside `model_version`, and state it on any accuracy report that leaves the building. A milestone must never be reachable by generating more data; if the ±15% claim would move by swapping synthetic for real, the claim is about our renderer, not our product.
...
4. **Cloud provider (AWS vs. Azure)** — **defaulted, not settled.** [ADR-0001](adr/0001-cloud-provider.md) picks AWS `ca-central-1` so the build can start, and records why: both regions satisfy residency, so residency does not decide it; the §3 stack already leans AWS (ECS/Fargate, Cognito). Reversal is near zero today and high once IAM and networking exist, so confirm or override **before** infra work starts. SR&ED is explicitly not a reason either way — that is a question for the founders' accountant, and if it favours Azure it should override the ADR.
...
6. **Ground-truth budget and calibration** — §5.4 deliberately does *not* depend on the 3 BC firms all sharing as-builts; it needs a funded self-measurement programme plus at most one cooperating firm as a calibration anchor. That converts an uncontrollable dependency into two answerable questions for the founders:
   ...
   - **Do we invest in simulated data, and when?** §5.4d is left open rather than committed. It is the one source that needs no site and no firm, so it is worth a small timeboxed evaluation during the Business Plan §16 spike — enough to learn whether BIM-derived renders transfer to real electrical rough-in photos at all. If they do, it de-risks cold start and shrinks the self-measured budget above; if they don't, we have spent a week and learned it early. Do not commit model-training effort to it before that evaluation reports, and do not let a promising early result quietly become the accuracy story.
