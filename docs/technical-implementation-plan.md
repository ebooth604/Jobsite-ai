...
4. **Training data:** starts from the 3 BC firms' real photo sets + as-built quantities referenced in Business Plan §16 (days 31–60 technical spike) — this is the seed dataset. Corrections logged in production become the ongoing training signal. **This dataset is a dependency, not a given** — the spike is what tests whether those firms will hand over as-builts at all. If they won't, there is no ground truth to train or measure against, and the Q1 accuracy milestone has nothing behind it. See §13.6.
5. **Accuracy targets from the roadmap (Business Plan §13):** ±15% by end of Q1, ±10% by end of Q2. Instrument accuracy measurement against held-out as-built ground truth, not just against internal cross-validation — the gap between the two is the real risk.
...
- **Accuracy harness:** automated comparison of model output against held-out as-built ground truth, tracked per trade, per model version, over time. This is the data behind the Q1/Q2 ±15%/±10% milestones and the "accuracy report" sales asset (Business Plan §5.2, §5.3). Note that the harness is only as real as the ground truth feeding it — if as-builts don't materialize (§13.6), internal cross-validation is all that's left, and it will flatter the model.
...
6. **Seed dataset availability** — §5.4 and the §11 accuracy harness both assume the 3 BC firms will share real photo sets *and* matching as-built quantities. The Business Plan §16 spike is what tests that assumption; it is not a delivery commitment, and it is not among the Business Plan §17 open questions. If as-builts don't come through, three things follow: there is no held-out ground truth, so accuracy can only be measured against internal cross-validation (which §5.5 explicitly warns is the wrong number); the Q1 ±15% milestone becomes unverifiable rather than merely unmet; and the fallback is a paid annotation pass over whatever photo sets *are* available, which costs money and calendar time not currently in the Q1 plan. Raise this with the founders before Q1 scheduling is locked, and treat "will they share as-builts?" as a discovery question with the same weight as "will the foreman take the photo?".
